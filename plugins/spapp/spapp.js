...
* Modified by: Widyanto Adinugroho <widyanto.adinugroho@gmail.com>
*/
...
(function($,window){
  ...
  function app(pageName,param) {
    ...
    var dynamic = $page.attr("dynamic");
    if(src && $page.find(">:first-child").length == 0) { // it has src and is a dynamic page
    ...
    } else if(typeof dynamic !== typeof undefined && dynamic !== false) { // it has src and is a dynamic page
      app.get(src, $page, pageName)
        .done(function(html){$page.html(html); show(pageName,param); })
        .fail(function(){ console.warn("failed to get %s page!",pageName);});
    } else
